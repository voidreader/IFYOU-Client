...
namespace Doozy.Editor.Common
{
	...
	// [CreateAssetMenu(menuName = "Doozy/Common/Product Info", fileName = "ProductInfo")]
	[Serializable]
	public class ProductInfo : ScriptableObject
	{
		...
		public static ProductInfo GetProduct(string category, string productName)
		{
#if UNITY_EDITOR
			string[] guids = AssetDatabase.FindAssets($"t:{nameof(ProductInfo)}");
			if (guids != null)
		...
		/// <param name="productName"> Name of the product to search for </param>
		public static ProductInfo GetProduct(string productName)
		{
#if UNITY_EDITOR
			string[] guids = AssetDatabase.FindAssets($"t:{nameof(ProductInfo)}");
			if (guids != null)
