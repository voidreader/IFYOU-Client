...
namespace NhnCloud.GamebaseTools.SettingTool
{
    public class GamebaseDependencies : IDisposable
    {
        ...
        public void UpdateGamebaseAllDependenciesFile(SettingToolCallback.ErrorDelegate callback)
        {
            ...
            {
                AddXmlChild(document, androidPackages, adapterSettings.android.fileName, "androidPackage", "spec", GamebaseInfo.GetCurrentVersion(SupportPlatform.ANDROID));
            }

            if (adapterSettings.useIOS == true)
            {
                AddXmlChild(document, iosPods, adapterSettings.ios.fileName, "iosPod", "name", GamebaseInfo.GetCurrentVersion(SupportPlatform.IOS));
            }
            ...
            string version)
        {
            ...
            switch (newNodeName)
            {
                ...
                case "iosPod":
                    {
                        // Gamebase SDK for iOS는 이슈 수정 시, 수정된 모듈만 패치 버전을 업데이트한다.
                        //
                        // iOS Patch Version은 항상 0으로 고정하고 
                        // cocoaPods 연산자(optimistic operator: ~>)를 사용하여 수정된 모듈만 업데이트될 수 있도록 처리한다.
                        // https://guides.cocoapods.org/using/the-podfile.html
                        
                        version = version.Replace(version.Substring(version.Length - 1, 1), "0");

                        var versionAttributes = document.CreateAttribute("version");
                        versionAttributes.Value = string.Format("~> {0}", version);
