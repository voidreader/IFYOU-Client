    ...
    public class GameServicesDemo : DemoActionPanelBase<GameServicesDemoAction, GameServicesDemoActionType>
    {
                ...
                case GameServicesDemoActionType.LoadLeaderboards:
                    GameServices.LoadLeaderboards((result, error) =>
                    {
                        if (error == null)
                        {
                            ...
                            {
                                var     eachLeaderboard    = leaderboards[iter];
                                Log(string.Format("[{0}]: {1}", iter, eachLeaderboard));
                                LoadLeaderboard(eachLeaderboard.Id);
                            }
                        }
                        else
                        {
        ...
        private void LoadLeaderboard(string leaderboardId)
        {
            ...
            leaderboard.LoadTopScores((result, error) =>
            {
                ...
                Log("Displaying scores..." + result.Scores.Length);
                foreach(IScore score in result.Scores)
                {
                    Log(string.Format("{0}", score));
                    Log(string.Format("{0}", score.Player));
                    Log(string.Format("{0}", score.Player.DisplayName));
                    Log(string.Format("{0}", score.Rank));
                    Log(string.Format("{0}", score.Value));
                }

                leaderboard.LoadImage((innerResult, innerError) =>
                {
                    if (innerError == null)
                        Debug.Log("Image data : " + innerResult.GetBytes(TextureEncodingFormat.JPG).Length);
                    else
                        Debug.LogError("Error loading leaderboard image : " + innerError);
                });
            });
        }
        #endregion
