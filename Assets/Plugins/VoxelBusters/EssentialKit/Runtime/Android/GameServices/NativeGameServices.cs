...
#endif
                Call(Native.Method.kAuthenticate, new object[] { listener, silentModeOnly } );
            });
        }
        public NativeGamePlayer GetLocalPlayer()
        {
#if NATIVE_PLUGINS_DEBUG_ENABLED
            DebugLogger.Log("[Class : NativeGameServices][Method : GetLocalPlayer]");
#endif
            AndroidJavaObject nativeObj = Call<AndroidJavaObject>(Native.Method.kGetLocalPlayer);
...
#endif
                Call(Native.Method.kLoadServerCredentials, new object[] { listener } );
            });
        }
