...
#endif
            Call(Native.Method.kRequestGalleryReadWriteAccess, new object[] { permissionInfo, listener } );
        }
...
#endif
                Call(Native.Method.kPickAssetFromGallery, new object[] { mimeType, pickerTitle, listener } );
            });
        }
...
#endif
                Call(Native.Method.kCaptureImageFromCamera, new object[] { title, listener } );
            });
        }
        public void SaveAssetToGallery(string folderName, NativeBytesWrapper assetData, NativeSaveAssetToGalleryListener listener)
        {
            Activity.RunOnUiThread(() => {
#if NATIVE_PLUGINS_DEBUG_ENABLED
                DebugLogger.Log("[Class : NativeMediaServices][Method(RunOnUiThread) : SaveAssetToGallery]");
#endif
                Call(Native.Method.kSaveAssetToGallery, new object[] { folderName, assetData.NativeObject, listener } );
            });
        }
...
#endif
                Call(Native.Method.kRequestGalleryReadAccess, new object[] { permissionInfo, listener } );
            });
        }
...
#endif
                Call(Native.Method.kRequestCameraAccess, new object[] { permissionInfo, listener } );
            });
        }
