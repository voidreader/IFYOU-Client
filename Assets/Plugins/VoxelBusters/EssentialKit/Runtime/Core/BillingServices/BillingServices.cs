    ...
    public static class BillingServices
    {
        ...
        internal static BillingProductDefinition FindProductDefinitionWithId(string id, bool returnObjectOnFail = false)
        {
            ...
            return returnObjectOnFail
                ? new BillingProductDefinition(id: kNullProductId, platformId: kNullProductId)
                : null;
        }
        ...
        {
            // validate arguments
            Assert.IsNotNullOrEmpty(productDefinitions, "productDefinitions");
        ...
        {
            // validate arguments
            Assert.IsArgNotNull(product, "product");
        ...
        public static bool BuyProduct(IBillingProduct product, int quantity = 1, string applicationUsername = null)
        {
            ...
            try
            {
                ...
                var     newPayment  = new BillingPayment(product.Id, product.PlatformId, quantity, applicationUsername);
        ...
        /// </remarks>
        /// <param name="applicationUsername">Application provided username that initiated this request. (optional)</param>
        public static void RestorePurchases(string applicationUsername = null)
        {
            try
            {
                // make request
                s_nativeInterface.RestorePurchases(applicationUsername);
            }
            catch (Exception exception)
            {
                DebugLogger.LogException(exception);
        ...
        private static bool IsProductOfType(string productId, BillingProductType type)
        {
            ...
            return (productDefinition != null) && (productDefinition.ProductType == type);
        }
