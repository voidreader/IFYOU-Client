...
namespace VoxelBusters.EssentialKit
{
    ...
    public static class CloudServices
    {
        ...
        public static KeyValueDataStore LocalCache => ObjectHelper.CreateInstanceIfNull(
        ...
        public static bool TryGetCloudAndLocalCacheValues<T>(string key, out T cloudValue, out T localCacheValue, T localCacheDefaultValue = default)
        {
            switch (Type.GetTypeCode(typeof(T)))
            {
                case TypeCode.Boolean:
                    cloudValue          = (T)(object)GetBool(key);
                    localCacheValue     = (T)(object)LocalCache.GetBool(key, (bool)(object)localCacheDefaultValue);
                    return true;

                case TypeCode.Int32:
                    cloudValue          = (T)(object)GetInt(key);
                    localCacheValue     = (T)(object)LocalCache.GetLong(key, (long)(object)localCacheDefaultValue);
                    return true;

                case TypeCode.Int64:
                    cloudValue          = (T)(object)GetLong(key);
                    localCacheValue     = (T)(object)LocalCache.GetLong(key, (long)(object)localCacheDefaultValue);
                    return true;

                case TypeCode.Single:
                    cloudValue          = (T)(object)GetFloat(key);
                    localCacheValue     = (T)(object)LocalCache.GetDouble(key, (double)(object)localCacheDefaultValue);
                    return true;

                case TypeCode.Double:
                    cloudValue          = (T)(object)GetDouble(key);
                    localCacheValue     = (T)(object)LocalCache.GetDouble(key, (double)(object)localCacheDefaultValue);
                    return true;

                case TypeCode.String:
                    cloudValue          = (T)(object)GetString(key);
                    localCacheValue     = (T)(object)LocalCache.GetString(key, (string)(object)localCacheDefaultValue);
                    return true;

                case TypeCode.Object:
                    if (typeof(T) == typeof(byte[]))
                    {
                        cloudValue      = (T)(object)GetByteArray(key);
                        localCacheValue = (T)(object)LocalCache.GetByteArray(key, (byte[])(object)localCacheDefaultValue);
                        return true;
                    }
                    else
                    {
