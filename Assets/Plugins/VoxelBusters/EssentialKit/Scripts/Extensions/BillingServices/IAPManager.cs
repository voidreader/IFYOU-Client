...
namespace VoxelBusters.EssentialKit.Extensions
{
    public static class IAPManager
    {
        ...
        public static void BuyProduct(string productId, EventCallback<IBillingTransaction> callback, string applicationUsername = null)
        {
            Assert.IsArgNotNull(callback, nameof(callback));
            ...
            var     initializing    = TryInitializeStore((products, error) =>
            {
                if (error == null)
                {
                    BuyProduct(productId, callback, applicationUsername);
                }
                else
                {
            ...
            if (!IsStoreAvailable())
            {
                callback.Invoke(null, BillingError.kStoreNotAvailable);
                return;
            }
            ...
            if (product == null)
            {
                callback.Invoke(null, BillingError.kProductNotFound);
                return;
            }
            ...
            BillingServices.BuyProduct(product, applicationUsername: applicationUsername);
        }

        public static void RestorePurchases(EventCallback<IBillingTransaction[]> callback, string applicationUsername = null)
        {
            Assert.IsArgNotNull(callback, nameof(callback));
            ...
            var     initializing    = TryInitializeStore((products, error) =>
            {
                if (error == null)
                {
                    RestorePurchases(callback, applicationUsername);
                }
                else
                {
                    callback.Invoke(null, error);
            ...
            {
                s_isRestoring   = true;
                BillingServices.RestorePurchases(applicationUsername);
            }
        }
        ...
        private static void HandleOnTransactionStateChange(BillingServicesTransactionStateChangeResult result)
        {
            ...
            foreach (var transaction in result.Transactions)
            {
                if (transaction.TransactionState == BillingTransactionState.Purchasing || 
                    transaction.TransactionState == BillingTransactionState.Restored)
                {
                    continue;
                }
